[![CocoaPods](https://img.shields.io/cocoapods/v/JetPaySDK_UI.svg?style=flat)](https://github.com/Jetpay/msdk-ios-ui)

JetPay SDK UI for iOS
===========
...
1.  Copy the `jetpaySDK.xcframework` file in the project folder of you iOS app.
2.  Add the library into your project. When using Xcode 12, you need to do the following:
    ...
    3.  Click +.
    4.  Click Add Other.
    5.  Select the `jetpaySDK.xcframework` file and click Add.
...
2.  Declare the reference to instance of `JetPaySDK` class in you app (for example, inside the `viewDidLoad` method):
    
        let sdkFacade = JetPayySDK()
...
2.  Declare the JetPaySDK library in you app (for example, inside the `viewDidLoad` method).
    
        JetPaySDK *sdkFacade = [[JetPaySDK alloc] init];
